      ...
      <PageFrame>
        ...
        <section className="py-20 lg:py-28 bg-[#f8f9fb]">
          <Container>
            ...
            <ul className="grid gap-4 sm:grid-cols-2">{solution.benefits.map((b, i) => (<li key={i} className="flex items-start gap-3 rounded-xl border border-[#e8eaed] bg-white p-5"><span className="mt-0.5 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-[#f5f0ff]"><Check className="h-3.5 w-3.5 text-[#7c5ce7]" strokeWidth={3} /></span><span className="text-[#525252]">{b}</span></li>))}</ul>
          </Container>
        </section>
        ...
        <section className="py-20 lg:py-28">
          <Container>
            ...
            <div className="grid gap-8 lg:grid-cols-3">
              ...
              <div className="flex flex-col justify-center"><h3 className="text-lg font-semibold text-[#0d0d0d]">How it works</h3><p className="mt-4 text-[#525252] leading-relaxed">{solution.approach}</p><div className="mt-6 flex flex-wrap gap-2">{solution.products.map((pid) => { const product = getProductById(pid); if (!product) return null; return (<span key={pid} className="rounded-full border border-[#e8eaed] px-3 py-1 text-xs font-medium text-[#737373]">{product.name}</span>); })}</div></div>
            </div>
          </Container>
        </section>
        <section className="py-20 lg:py-28 bg-[#f8f9fb]">
