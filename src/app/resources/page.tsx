    ...
    <>
      <section className="py-16 lg:py-24 nav-dashed-bottom">
        <Container>
          ...
          <h1 className="mt-3 max-w-2xl text-3xl font-bold tracking-[-0.02em] text-ink lg:text-4xl">Practical guides for infrastructure decision-makers operating in China</h1>
          <p className="mt-4 max-w-2xl text-lg text-subtle">Technical, vendor-neutral resources to help you design, deploy, and operate infrastructure in China.</p>
        </Container>
      </section>
        ...
        <section className="py-20 lg:py-28 nav-dashed-bottom">
          <Container>
            ...
            <div className="mt-6 grid gap-6 md:grid-cols-2">
              {featured.map((f) => (
                <div key={f.title} className="flex flex-col rounded-xl border border-border hover:border-brand bg-white p-8 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
                  <span className="inline-flex self-start items-center gap-1.5 rounded-md bg-surface px-2.5 py-1 text-xs font-medium text-muted"><f.icon className="h-3.5 w-3.5" />{f.type}</span>
                  <h3 className="mt-4 text-lg font-bold text-ink">{f.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-muted">{f.excerpt}</p>
                  <span className="mt-auto pt-4 inline-flex items-center gap-1.5 text-sm font-medium text-ink">Coming soon<ArrowRight className="h-4 w-4" /></span>
                </div>
              ))}
            </div>
          </Container>
        ...
        <section className="py-20 lg:py-28 nav-dashed-bottom">
          <Container>
            ...
            <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {guides.map((g) => (
                <div key={g.title} className="flex flex-col rounded-xl border border-border hover:border-brand bg-white p-6 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
